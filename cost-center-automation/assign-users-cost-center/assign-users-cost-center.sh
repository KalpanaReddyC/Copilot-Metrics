...
# Script to add users from CSV file to a GitHub Enterprise cost center
...
load_env() {
    ...
    if [[ -z "$env_file" ]]; then
        ...
        if [[ -f "../.env" ]]; then
            ...
            echo "  USERS_CSV_FILE=path/to/users.csv"
    ...
    local required_vars=("GITHUB_ENTERPRISE" "COST_CENTER_ID" "USERS_CSV_FILE" "GITHUB_TOKEN")
    local missing_vars=()
...
extract_users_from_csv() {
    ...
    log_info "Found ${#users[@]} users in CSV file" >&2
    printf '%s\n' "${users[@]}"
}
...
add_users_to_cost_center() {
    ...
    log_info "Adding ${#users[@]} users to cost center '$cost_center_id'"
    ...
    if [[ -z "$response" ]]; then
        log_error "Failed to add users to cost center - no response received"
        return 1
    fi
    ...
    if echo "$response" | jq -e .message >/dev/null 2>&1; then
        ...
        if [[ "$message" == *"successfully"* ]]; then
            log_success "Users added successfully to cost center"
        else
    ...
    if echo "$response" | jq -e .message >/dev/null 2>&1; then
        ...
        # Check for reassigned resources
        if echo "$response" | jq -e .reassigned_resources >/dev/null 2>&1; then
            local reassigned_count=$(echo "$response" | jq '.reassigned_resources | length')
            if [[ $reassigned_count -gt 0 ]]; then
                log_warning "Some resources were reassigned from other cost centers:"
                echo "$response" | jq -r '.reassigned_resources[] | "  - \(.name) (\(.resource_type)) from \(.previous_cost_center)"'
            fi
        fi
    else
        log_info "Raw API Response: $response"
    fi
}
...
Add users from a CSV file to a GitHub Enterprise cost center.
...
ENVIRONMENT VARIABLES (in .env file):
    ...
    COST_CENTER_ID      Cost center ID to add users to (required)
    USERS_CSV_FILE      Path to CSV file containing usernames (required)
...
main() {
    ...
    log_info "Starting GitHub Cost Center User Assignment Script"
    ...
    log_info "Processing CSV file: $USERS_CSV_FILE"
    local users
    mapfile -t users < <(extract_users_from_csv "$USERS_CSV_FILE")
    ...
    log_info "Users to add: ${users[*]}"
    
    # Confirm before proceeding
    echo
    log_warning "About to add ${#users[@]} users to cost center '$COST_CENTER_ID' in enterprise '$GITHUB_ENTERPRISE'"
    ...
    add_users_to_cost_center "$GITHUB_ENTERPRISE" "$COST_CENTER_ID" "${users[@]}"
